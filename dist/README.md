...
- Toggles OAM Webgate-only traffic for browser-to-OAM and browser-to-WebGate communication. The filter checks URLs, headers, cookies, and bodies, while endpoint badges use URL paths, redirect roles, and learned host-plus-port roles. OAM cookies remain highlighted as artifacts without incorrectly changing a WebGate endpoint into an OAM endpoint.
