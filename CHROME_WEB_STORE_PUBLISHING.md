...
- Upload archive: `oracle-sso-devtools-v2.0.0.zip`
...
The current feature release is version `2.0.0`. For later releases, increase the version in both `package.json` and the root `manifest.json`, for example, to `2.0.1` or `2.1.0`, then regenerate `dist/`. Chrome rejects an uploaded package if its version is not higher than the previously uploaded version.
...
npm test
cd dist
zip -r ../oracle-sso-devtools-v2.0.0.zip \
...
```bash
unzip -l oracle-sso-devtools-v2.0.0.zip
unzip -p oracle-sso-devtools-v2.0.0.zip manifest.json
```
...
3. Upload `oracle-sso-devtools-v2.0.0.zip`.
