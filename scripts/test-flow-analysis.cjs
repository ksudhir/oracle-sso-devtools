...
assert.match(serviceWorkerSource, /panel\.html\?mode=viewer/u);
assert.match(panelMarkup, /id="runtimeModeLabel" class="runtimeModeLabel" hidden>Offline Viewer/u);
assert.match(panelMarkup, /id="viewerThemeSelect" aria-label="Offline Viewer theme"/u);
assert.match(panelMarkup, /<option value="system">System<\/option>/u);
assert.match(panelStyles, /:root\[data-theme="dark"\]/u);
assert.match(panelStyles, /:root\[data-theme="light"\]/u);
assert.match(fs.readFileSync(panelPath, "utf8"), /authInspector\.offlineViewerTheme/u);
assert.match(panelStyles, /\.shell\.isOfflineViewerEmpty/u);
assert.match(panelStyles, /\.offlineViewerWelcome/u);
