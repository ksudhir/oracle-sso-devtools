...
const archiveName = `enterprise-auth-netlog-inspector-chrome-v${packageJson.version}.zip`;
const archivePath = path.join(projectRoot, archiveName);

run(process.execPath, [path.join(__dirname, "build-dist.cjs")], projectRoot);
